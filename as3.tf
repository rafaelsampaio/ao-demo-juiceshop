data "http" "as3_template" {
  url = "https://raw.githubusercontent.com/rafaelsampaio/ao-demo-juiceshop/main/juiceshop-basic.json.tftpl"
  #url = "https://raw.githubusercontent.com/rafaelsampaio/ao-demo-juiceshop/main/juiceshop-full.json.tftpl"
}

locals {
  juiceshop-vars = {
    app_tenant    = "ecommerce"
    app_name      = "juiceshop"
    app_address   = var.app-address
    app_node_port = "8000"

    #Basic App (appBasic.json.tpl)
    app_nodes = ["10.1.20.2", "10.1.20.3"]

    #Full Application Services (appServiceDiscoveryTLS.json.tpl)
    #app_tag         = "rsampaio-app"
    #app_region      = "southamerica-east1"
    #app_certificate = replace(tls_self_signed_cert.juiceshop-certificate.cert_pem, "/\n/", "\\n")
    #app_private_key = replace(tls_private_key.juiceshop-private-key.private_key_pem, "/\n/", "\\n")
    #app_waf = "https://raw.githubusercontent.com/rafaelsampaio/ao-demo-juiceshop/main/waf-policy-v1.json"
    #network_dos_vectors = local.dos-network-vectors-settings
  }

  juiceshop-as3 = templatefile(data.http.as3_template.response_body, local.juiceshop-vars)
}

resource "bigip_as3" "juiceshop" {
  as3_json = local.juiceshop-as3
}

